...
export class Edge<Vertex extends vertexid | vertexname> {
    from: Vertex
    to: Vertex
...
export class Graph {
    vertexNames: vertexname[]
    edges: Edge<vertexid>[]
    degrees: number[]

    // Constructor converts string-based graph to index-base graph
    constructor(vertices: vertexname[], edges: Edge<vertexname>[]) {
        ...
        edges.forEach(edge => {
            ...
            if (!this.hasEdge(forward)) this.edges.push(forward)
            if (!this.hasEdge(backward)) this.edges.push(backward)
        })
        this.degrees = []
    ...
    hasEdge(a: Edge<vertexid>): boolean {
        for (let edge of this.edges)
            if (edge.from == a.from &&edge.to == a.to) return true
        return false
    }

    getEulerWalk(): string[] | null {
        let [start, end] = [null, null]    
        for (let i = 0; i < this.degrees.length; i++) {
            if (this.degrees[i] % 2 == 0) continue // Not odd degree
            if (start === null) start = i // First odd degree
            else if (end === null) end = i // Second odd degree
            else return null // Third odd degree
        }
        if (start === null && end !== null) return null // Only one odd degree
        ...
        if (walk.length != (this.edges.length / 2)) return null
        if (!isEven) walk.push(current) // If its not an Euler circle, current element is not in included yet
        return walk.map(vertex => this.vertexNames[vertex])
    }

    walkDFS(walk: vertexid[], current: vertexid): vertexid[] | null{
        walk = walk.concat([current])
        if (walk.length == this.vertexNames.length) return walk
    ...
    getHamiltonWalk(): string[] | null {
        for (let vertid = 0; vertid < this.vertexNames.length; vertid++) {
            let result = this.walkDFS([], vertid)
            if (result !== null) return result.map(vertex => this.vertexNames[vertex])
        }
        return null
    }
}
